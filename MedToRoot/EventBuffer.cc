...
// 	  cout << "added coincident adc: " << moduleNumber << " time " << adcSubEvent->Timestamp() << " #ch: " << adcSubEvent->AdcChannel().size() << " #values: " << adcSubEvent->AdcValue().size() << endl;
	  return BuiltEvents.size();
	}
    }

  BuiltEvents.push_back(BuiltEvent(moduleNumber, adcSubEvent, adcSubEvent->Timestamp(), LaserOn, FieldUp, FieldDown));
//   cout << "added adc: " << moduleNumber << " time " << adcSubEvent->Timestamp() << " #ch: " << adcSubEvent->AdcChannel().size() << " #values: " << adcSubEvent->AdcValue().size() << endl;
...
//   cout << endl << "Adding Gamma!" << endl << endl;
...
void EventBuffer::Sort()
{
  ...
  for(i = 1; i < BuiltEvents.size(); i++)
      ...
      for(j = 0; j < i; j++)
	{
	  if(BuiltEvents[i].GetTime() < BuiltEvents[SubEventNumber[j]].GetTime())
	      ...
	      for(k = i; k > j; k--)
		{
		  ...
		  if(Settings->VerboseLevel() > 4)
		    {
		      cout<<"i = "<<i<<", j = "<<j<<", k = "<<k<<", SubEventNumber["<<k<<"] = "<<SubEventNumber[k]<<", SubEventNumber["<<k-1<<"] = "<<SubEventNumber[k-1]<<endl;
		    }
		}
	      break;
	    }
      ...
      if(Settings->VerboseLevel() > 4)
	{
	  cout<<"i = "<<i<<", j = "<<j<<", SubEventNumber["<<j<<"] = "<<SubEventNumber[j]<<endl;
	}
    }

  if(Settings->VerboseLevel() > 3)
    {
      cout<<"After sorting "<<BuiltEvents.size()<<" got following order (first line = index, second line = time):"<<endl;
