...
GlobalSettings::GlobalSettings( unsigned int argc, char* argv[] ) {
	...
	if( fMedFile.empty() ) {
		...
		cerr << "Error, name of the MED-file (" << fMedFile << ") not provided!" << endl;
		exit(1);
		...
		cerr << "Error, name of the settings-file (" << fSettingsFile << ") not provided!" << endl;
		exit(1);
...
bool GlobalSettings::Verify() {
	...
	for( unsigned short i = 0; i < fNumberOfTimestampModules; i++ ) {
		...
		if( fTimestampModule[i] > fNumberOfDgfModules ) {
			...
			cerr << __PRETTY_FUNCTION__ << ": Error, inconsistent settings: TimestampModule ";
			cerr << i << " = " << fTimestampModule[i] << ", but total number of dgf modules is only ";
