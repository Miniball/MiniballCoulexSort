...
void hists::Initialise() {
	...
	for( int i = 0; i < 4; i++ ) { // per CD quadrant
		...
		for( int j = 0; j < 24; j++ ) { // per Ge detector
			...
			hname = "T_dcB_x" + doppler::convertInt(i) + "_" + doppler::convertInt(j);
			htitle = "Target gated (Q" + doppler::convertInt(i) + ", Ge" + doppler::convertInt(j) + "), background subtracted gamma rays, Doppler corrected for scattered projectile;Energy [keV];Counts per 1keV";
			T_dcB_x[(int)(24*i+j)] = new TH1F(hname.c_str(),htitle.c_str(),GBINS,-0.5*((float)GMAX/(float)GBINS),GMAX-0.5*((float)GMAX/(float)GBINS));	
		
			hname = "T_dcT_x" + doppler::convertInt(i) + "_" + doppler::convertInt(j);
			htitle = "Target gated (Q" + doppler::convertInt(i) + ", Ge" + doppler::convertInt(j) + "), background subtracted gamma rays, Doppler corrected for target recoil;Energy [keV];Counts per 1keV";
			T_dcT_x[(int)(24*i+j)] = new TH1F(hname.c_str(),htitle.c_str(),GBINS,-0.5*((float)GMAX/(float)GBINS),GMAX-0.5*((float)GMAX/(float)GBINS));	
		
			hname = "B_dcB_x" + doppler::convertInt(i) + "_" + doppler::convertInt(j);
			htitle = "Beam gated (Q" + doppler::convertInt(i) + ", Ge" + doppler::convertInt(j) + "), background subtracted gamma rays, Doppler corrected for scattered projectile;Energy [keV];Counts per 1keV";
			B_dcB_x[(int)(24*i+j)] = new TH1F(hname.c_str(),htitle.c_str(),GBINS,-0.5*((float)GMAX/(float)GBINS),GMAX-0.5*((float)GMAX/(float)GBINS));	
		
			hname = "B_dcT_x" + doppler::convertInt(i) + "_" + doppler::convertInt(j);
			htitle = "Beam gated (Q" + doppler::convertInt(i) + ", Ge" + doppler::convertInt(j) + "), background subtracted gamma rays, Doppler corrected for target recoil;Energy [keV];Counts per 1keV";
			B_dcT_x[(int)(24*i+j)] = new TH1F(hname.c_str(),htitle.c_str(),GBINS,-0.5*((float)GMAX/(float)GBINS),GMAX-0.5*((float)GMAX/(float)GBINS));	
...
void hists::FillGam1h( float GEn, float GTh, float GPh, int cid, float PEn, Int_t Pann,
	...
	if( cut == 0 ) {
		...
		T_dcB_x[(int)(Pquad*24+cid)]->Fill(GEn*doppler::DC(BEn, BTh, BPh, GTh, GPh, AP), weight);	
		T_dcT_x[(int)(Pquad*24+cid)]->Fill(GEn*doppler::DC(TEn, TTh, TPh, GTh, GPh, AT), weight);
	...
	// Projectile
		...
		B_dcB_x[(int)(Pquad*24+cid)]->Fill(GEn*doppler::DC(BEn, BTh, BPh, GTh, GPh, AP), weight);	
		B_dcT_x[(int)(Pquad*24+cid)]->Fill(GEn*doppler::DC(TEn, TTh, TPh, GTh, GPh, AT), weight);
...
void hists::FillGam2h( float GEn, float GTh, float GPh, int cid, vector<float> PEn, vector<int> Pann,
	...
	T_dcB_x[(int)(Pquad[Tptr]*24+cid)]->Fill(GEn*doppler::DC(BEn, BTh, BPh, GTh, GPh, AP), weight);	
	T_dcT_x[(int)(Pquad[Tptr]*24+cid)]->Fill(GEn*doppler::DC(TEn, TTh, TPh, GTh, GPh, AT), weight);

	B_dcB_x[(int)(Pquad[Bptr]*24+cid)]->Fill(GEn*doppler::DC(BEn, BTh, BPh, GTh, GPh, AP), weight);	
	B_dcT_x[(int)(Pquad[Bptr]*24+cid)]->Fill(GEn*doppler::DC(TEn, TTh, TPh, GTh, GPh, AT), weight);
